...
class Point():
    ...
    def __str__(self):
        return f"{self.x} {self.y} walked distance: {self.distance_walked}, Manhattan distance: {self.x + self.y}"
...
def visualize():
    ...
    for wire in wires:
        ...
        paths.append(Path(positions, movement_types))

    patch = patches.PathPatch(paths[0], lw=1, facecolor="none")
    ax.add_patch(patch)
    patch = patches.PathPatch(
        paths[1], lw=1, facecolor="none", edgecolor="red")
    ax.add_patch(patch)
...
if __name__ == "__main__":
    ...
    closest = sorted(intersections)
    least_distance = sorted(intersections, key=lambda x: x.distance_walked)
    print(f"Nearest intersection: {closest[1]}")
    print(
        f"\"Shortest\" way to an intersection is {least_distance[1].distance_walked}")
